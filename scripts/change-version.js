...
async function init(version, versionBuild, ask = true) {
  ...
  const files = [
    ...
    `${__dirname}/../ios/socilApp/Info.plist`
  ];
  ...
  await execCommand(`echo VERSION=${versionBase}.${versionBuild} > ${__dirname}/.lastVersion`);
  await execCommand(`echo VERSION_BUILD=${versionBuild} >> ${__dirname}/.lastVersion`);
